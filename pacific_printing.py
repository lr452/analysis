...
cube1_s = np.mean(cube1_average.data[17:18])#.collapsed(['latitude'],iris.analysis.MEAN,weights=grid_areas)
...
max_depth = 100.0
indexes = np.where(seasonal_average3.coord('ocean model level').points <= max_depth)[0]
seasonal_average3 = seasonal_average3[indexes]
average_across_depth3 = seasonal_average3.collapsed(['ocean model level'],iris.analysis.MEAN)
...
cube_months6 = cube6[np.where((cube2.coord('month_number').points == 6) | (cube6.coord('month_number') == 7) | (cube6.coord('month_number') == 8))]
